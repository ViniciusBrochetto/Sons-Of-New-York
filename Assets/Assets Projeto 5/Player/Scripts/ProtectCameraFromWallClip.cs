...
public class ProtectCameraFromWallClip : MonoBehaviour
{
    ...
    Vector3 m_OriginalDir;
    Vector3 m_CurrDir;

    public Transform m_CamOriginalPosition;
    ...
    private void Start()
    {
        ...
        m_OriginalDir = m_Pivot.position - m_CamOriginalPosition.position;
        m_OriginalDir = m_OriginalDir / m_OriginalDir.magnitude;
    ...
    private void LateUpdate()
    {
        ...
        m_CurrDir = m_Pivot.position - m_CamOriginalPosition.position;

        m_Ray.origin = m_Pivot.position + m_CurrDir * sphereCastRadius;
        m_Ray.direction = -m_CurrDir;
        ...
        if (initialIntersect)
        {
            m_Ray.origin += m_CurrDir * sphereCastRadius;

            Debug.DrawRay(m_Ray.origin, m_Ray.direction, Color.cyan, 0.1f);
        ...
        if (hitSomething)
        {
            Debug.DrawRay(m_Ray.origin, -m_CurrDir * (targetDist + sphereCastRadius), Color.red);
        }
        ...
        m_Cam.localPosition = -m_OriginalDir * m_CurrentDist;
        ...
        if (GetComponentInParent<PhotonView>() == null || GetComponentInParent<PhotonView>().isMine)
        {
            ...
            {
                if (Vector3.Distance(m_Cam.position, player.position) < 2.2f)
                    r.enabled = false;
                else
                    r.enabled = true;
            }
