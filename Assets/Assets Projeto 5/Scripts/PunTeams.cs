...
public enum Team : byte {TeamA, TeamB };
...
static class TeamExtensions
{
    ...
    public static void SetTeam(this PhotonPlayer player, Team team)
    {
        ...
        Team currentTeam = PhotonNetwork.player.GetTeam();
        if (currentTeam != team)
        {
    ...
    public static void SwitchTeam(this PhotonPlayer player)
    {
        if (!PhotonNetwork.connectedAndReady)
        {
            Debug.LogWarning("JoinTeam was called in state: " + PhotonNetwork.connectionStateDetailed + ". Not connectedAndReady.");
        ...
        else if (currentTeam == Team.TeamB)
            currentTeam = Team.TeamA;
        else
            currentTeam = Team.TeamA;

        PhotonNetwork.player.SetCustomProperties(new Hashtable() { { PunTeams.TeamPlayerProp, currentTeam } });
    }
}