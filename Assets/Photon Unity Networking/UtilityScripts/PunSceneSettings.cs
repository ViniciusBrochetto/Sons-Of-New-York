...
#if UNITY_EDITOR
            instanceField = (PunSceneSettings)UnityEditor.AssetDatabase.LoadAssetAtPath(PunSceneSettingsCsPath, typeof(PunSceneSettings));
            if (instanceField == null)
            {
                instanceField = ScriptableObject.CreateInstance<PunSceneSettings>();
                UnityEditor.AssetDatabase.CreateAsset(instanceField, PunSceneSettingsCsPath);
            }
#endif
