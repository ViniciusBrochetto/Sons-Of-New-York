...
public class PickupDemoGui : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        if (this.ShowScores)
        {
            GUILayout.Label("Your Score: " + PhotonNetwork.player.GetKills());
        }
        ...
        if (this.ShowTeams)
        {
            foreach (var teamName in PunTeams.PlayersPerTeam.Keys)
            {
                ...
                {
                    GUILayout.Label("  " + player.ToStringFull() + " Score: " + player.GetKills());
                }
            }

            if (GUILayout.Button("to red"))
            {
                PhotonNetwork.player.SetTeam(Team.TeamA);
            }
            if (GUILayout.Button("to blue"))
            {
                PhotonNetwork.player.SetTeam(Team.TeamB);
            }
        }
    }
}
